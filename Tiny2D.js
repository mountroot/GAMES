...
function RectangleEntity(x, y, width, height) {
    ...
    this.isHit = function(i, j) {
        return(this.x <= i && i <= this.x + this.w && this.y <= j && this.y + this.h)
    }
}
...
function CircleEntity(x, y, radius, type, restitution, deceleration) {
    ...
    this.collidedWithRect = function(r) {  // 円と短形の衝突
        // 短形の4辺上で最も近い座標(nx, ny)を求める
        var nx = Math.max(r.x, Math.min(this.x, r.x + r.w));
        var ny = Math.max(r.y, Math.min(this.y, r.y + r.h));
        
        if(!this.isHit(nx, ny)) {          // 衝突無し → リターン
            return;
        }
    ...
    this.collidedWithCircle = function(peer) {  // 円と縁の衝突
        ...
        else if(peer.type == BodyDynamic && peer.type == BodyStatic) {
